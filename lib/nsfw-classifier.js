...
const APPEARANCE_TOKENS = [
    ...
    'wavy hair', 'straight hair', 'curly hair', 'updo', 'high bun', 'low bun', 'messy bun', 'space buns',
    'ponytail', 'twintails', 'braid', 'braids', 'pigtails', 'side ponytail',
    'low ponytail', 'high ponytail', 'short ponytail', 'french braid', 'fishtail braid', 'side braid',
    'chignon', 'hair bun', 'half updo',
    // v0.11.22 hair state / motion — AI 在 NSFW 场景常加，覆盖 anchor 发型（hair down / wet hair 等），
    // 加进列表让 stripAppearanceTokens 剥掉，保 anchor 发型权威。
    'loose hair', 'messy hair', 'tousled hair', 'disheveled hair', 'wild hair', 'unkempt hair',
    'hair down', 'hair up', 'tied hair', 'untied hair', 'undone hair', 'pulled back hair', 'pulled hair',
    'hair flowing', 'flowing hair', 'flying hair', 'floating hair', 'windblown hair', 'tangled hair',
    'wet hair', 'damp hair', 'sweaty hair', 'dripping hair', 'oily hair', 'sleek hair', 'shiny hair',
    'hair over face', 'hair covering face', 'hair over one eye', 'hair in face',
    'bangs', 'side bangs', 'blunt bangs', 'curtain bangs', 'swept bangs', 'long bangs', 'short bangs',
    'parted hair', 'side part', 'middle part', 'center part',
    // eyes
...
// v0.11.18 检测 AI prompt 中是否包含明确的服装 tag
// 用途：若用户说"穿帝袍/换西装/着旗袍"等 → AI prompt 含 outfit token → 锚点的原始服装需被剥离避免冲突
// （否则锚点的 "dark blue armor, cape, boots" + AI 的 "emperor robe" 同时塞进模型 → 出钢架军装 + 龙袍混搭畸形）
export function hasOutfitTokens(prompt) {
    if (!prompt) return false;
    const lowerTokens = OUTFIT_TOKENS.map((t) => t.toLowerCase());
    const parts = prompt.split(',').map((p) => p.trim()).filter(Boolean);
    return parts.some((tag) => {
        const low = tag.toLowerCase().replace(/[()]/g, '').replace(/:[\d.]+/g, '').trim();
        return lowerTokens.some((k) => low === k || low.endsWith(' ' + k));
    });
}
