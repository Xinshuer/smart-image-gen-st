...
export const workflowTemplates = {
    ...
    // NoobAI vPred + EasyNegative LoRA (loaded with negative UNet weight, per author guidance).
    // Pipeline: ckpt → ModelSamplingDiscrete(v_prediction) → RescaleCFG(0.7)
    //         → LoraLoader(detailer, +0.5)
    //         → LoraLoader(EasyNegative, strength_model=-0.6, strength_clip=0)  ← negative UNet only
    //         → FBCache → KSampler → FaceDetailer → upscale.
    // The clip strength is 0 because applying an "easy negative" LoRA to the text encoder
    // would corrupt prompt understanding; we only want it to subtract bad-quality features
    ...
    noobai_easyneg: {
        ...
        "101": { "inputs": { "model": ["100", 0], "multiplier": 0.7 }, "class_type": "RescaleCFG" },
        "2":   { "inputs": { "model": ["101", 0], "clip": ["1", 1], "lora_name": "NoobAI-XL (NAI-XL)的LoRA_动漫风格\\NOOB_vp1_detailer_v1.safetensors", "strength_model": 0.5, "strength_clip": 0.5 }, "class_type": "LoraLoader" },
        "3":   { "inputs": { "model": ["2", 0], "clip": ["2", 1], "lora_name": "NoobAI-XL (NAI-XL)的LoRA_动漫风格\\noobaiXL_EasyNegative_LoRA-3.0_last.safetensors", "strength_model": -0.6, "strength_clip": 0.0 }, "class_type": "LoraLoader" },
        "6":   { "inputs": { "model": ["3", 0], "object_to_patch": "diffusion_model", "residual_diff_threshold": 0.15, "start": 0.2, "end": 0.8, "max_consecutive_cache_hits": 5 }, "class_type": "ApplyFBCacheOnModel" },
        "7":   { "inputs": { "clip": ["3", 1], "text": "%prompt%" }, "class_type": "CLIPTextEncode" },
        "8":   { "inputs": { "clip": ["3", 1], "text": "%negative_prompt%" }, "class_type": "CLIPTextEncode" },
        "9":   { "inputs": { "width": 832, "height": 1216, "batch_size": 1 }, "class_type": "EmptyLatentImage" },
        ...
        "11":  { "inputs": { "samples": ["10", 0], "vae": ["1", 2] }, "class_type": "VAEDecode" },
        "30":  { "inputs": { "model_name": "bbox/face_yolov8m.pt" }, "class_type": "UltralyticsDetectorProvider" },
        "31":  { "inputs": { "image": ["11", 0], "model": ["6", 0], "clip": ["3", 1], "vae": ["1", 2], "guide_size": 512.0, "guide_size_for": true, "max_size": 1024.0, "seed": 12345, "steps": 20, "cfg": 7.0, "sampler_name": "euler", "scheduler": "normal", "positive": ["7", 0], "negative": ["8", 0], "denoise": 0.45, "feather": 5, "noise_mask": true, "force_inpaint": true, "bbox_threshold": 0.5, "bbox_dilation": 10, "bbox_crop_factor": 3.0, "sam_detection_hint": "center-1", "sam_dilation": 0, "sam_threshold": 0.93, "sam_bbox_expansion": 0, "sam_mask_hint_threshold": 0.7, "sam_mask_hint_use_negative": "False", "drop_size": 10, "bbox_detector": ["30", 0], "wildcard": "", "cycle": 1 }, "class_type": "FaceDetailer" },
        "27":  { "inputs": { "model_name": "RealESRGAN_x2plus.pth" }, "class_type": "UpscaleModelLoader" },
        "28":  { "inputs": { "upscale_model": ["27", 0], "image": ["31", 0] }, "class_type": "ImageUpscaleWithModel" },
        "12":  { "inputs": { "images": ["28", 0], "filename_prefix": "Noobai_EasyNeg_2K" }, "class_type": "SaveImage" }
    },

    // NoobAI vPred + EasyNegative (-0.6) + miaomiaoHarem style LoRA (+1.0).
    // miaomiaoHarem-vpred12 is a style/illustration LoRA (mecha musume artist nidy2d). No trigger words.
    // Stacks with EasyNegative for the full quality + style combo, on top of vPred sampling.
    noobai_miaomiao: {
        "1":   { "inputs": { "ckpt_name": "noobaiXLNAIXL_vPred10Version.safetensors", "clip_skip": "2" }, "class_type": "CheckpointLoaderSimple" },
        "100": { "inputs": { "model": ["1", 0], "sampling": "v_prediction", "zsnr": true }, "class_type": "ModelSamplingDiscrete" },
        "101": { "inputs": { "model": ["100", 0], "multiplier": 0.7 }, "class_type": "RescaleCFG" },
        "2":   { "inputs": { "model": ["101", 0], "clip": ["1", 1], "lora_name": "NoobAI-XL (NAI-XL)的LoRA_动漫风格\\NOOB_vp1_detailer_v1.safetensors", "strength_model": 0.5, "strength_clip": 0.5 }, "class_type": "LoraLoader" },
        "3":   { "inputs": { "model": ["2", 0], "clip": ["2", 1], "lora_name": "NoobAI-XL (NAI-XL)的LoRA_动漫风格\\noobaiXL_EasyNegative_LoRA-3.0_last.safetensors", "strength_model": -0.6, "strength_clip": 0.0 }, "class_type": "LoraLoader" },
        "4":   { "inputs": { "model": ["3", 0], "clip": ["3", 1], "lora_name": "NoobAI-XL (NAI-XL)的LoRA_动漫风格\\style-nidy2d-miaomiaoHarem_vpred12_v01.safetensors", "strength_model": 1.0, "strength_clip": 1.0 }, "class_type": "LoraLoader" },
        ...
        "11":  { "inputs": { "samples": ["10", 0], "vae": ["1", 2] }, "class_type": "VAEDecode" },
        "30":  { "inputs": { "model_name": "bbox/face_yolov8m.pt" }, "class_type": "UltralyticsDetectorProvider" },
        "31":  { "inputs": { "image": ["11", 0], "model": ["6", 0], "clip": ["4", 1], "vae": ["1", 2], "guide_size": 512.0, "guide_size_for": true, "max_size": 1024.0, "seed": 12345, "steps": 20, "cfg": 7.0, "sampler_name": "euler", "scheduler": "normal", "positive": ["7", 0], "negative": ["8", 0], "denoise": 0.45, "feather": 5, "noise_mask": true, "force_inpaint": true, "bbox_threshold": 0.5, "bbox_dilation": 10, "bbox_crop_factor": 3.0, "sam_detection_hint": "center-1", "sam_dilation": 0, "sam_threshold": 0.93, "sam_bbox_expansion": 0, "sam_mask_hint_threshold": 0.7, "sam_mask_hint_use_negative": "False", "drop_size": 10, "bbox_detector": ["30", 0], "wildcard": "", "cycle": 1 }, "class_type": "FaceDetailer" },
        "27":  { "inputs": { "model_name": "RealESRGAN_x2plus.pth" }, "class_type": "UpscaleModelLoader" },
        "28":  { "inputs": { "upscale_model": ["27", 0], "image": ["31", 0] }, "class_type": "ImageUpscaleWithModel" },
        "12":  { "inputs": { "images": ["28", 0], "filename_prefix": "Noobai_Miaomiao_2K" }, "class_type": "SaveImage" }
    },

    // WAI-Illustrious-SDXL v17 (作者: WAI) — Illustrious-XL 体系的稳定动漫微调，VAE 内嵌。
    // 作者推荐参数: Steps 15-30, CFG 5-7, Sampler Euler a (ancestral), size ≥1024x1024。
    // 评级标签 4 档: general / sensitive / nsfw / explicit (在 buildPrompt 中按 SFW/NSFW 注入)。
    // 不需 ZSNR / RescaleCFG (非 vPred 模型)。
    wai_illustrious: {
        "1": { "inputs": { "ckpt_name": "waiIllustriousSDXL_v170.safetensors", "clip_skip": "2" }, "class_type": "CheckpointLoaderSimple" },
        ...
        "9": { "inputs": { "width": 1024, "height": 1344, "batch_size": 1 }, "class_type": "EmptyLatentImage" },
        "10": { "inputs": { "model": ["6", 0], "positive": ["7", 0], "negative": ["8", 0], "latent_image": ["9", 0], "seed": 0, "steps": 25, "cfg": 6.0, "sampler_name": "euler_ancestral", "scheduler": "normal", "denoise": 1.0 }, "class_type": "KSampler" },
        "11": { "inputs": { "samples": ["10", 0], "vae": ["1", 2] }, "class_type": "VAEDecode" },
        "30": { "inputs": { "model_name": "bbox/face_yolov8m.pt" }, "class_type": "UltralyticsDetectorProvider" },
        "31": { "inputs": { "image": ["11", 0], "model": ["6", 0], "clip": ["1", 1], "vae": ["1", 2], "guide_size": 512.0, "guide_size_for": true, "max_size": 1024.0, "seed": 12345, "steps": 20, "cfg": 6.0, "sampler_name": "euler_ancestral", "scheduler": "normal", "positive": ["7", 0], "negative": ["8", 0], "denoise": 0.45, "feather": 5, "noise_mask": true, "force_inpaint": true, "bbox_threshold": 0.5, "bbox_dilation": 10, "bbox_crop_factor": 3.0, "sam_detection_hint": "center-1", "sam_dilation": 0, "sam_threshold": 0.93, "sam_bbox_expansion": 0, "sam_mask_hint_threshold": 0.7, "sam_mask_hint_use_negative": "False", "drop_size": 10, "bbox_detector": ["30", 0], "wildcard": "", "cycle": 1 }, "class_type": "FaceDetailer" },
        "27": { "inputs": { "model_name": "RealESRGAN_x2plus.pth" }, "class_type": "UpscaleModelLoader" },
        "28": { "inputs": { "upscale_model": ["27", 0], "image": ["31", 0] }, "class_type": "ImageUpscaleWithModel" },
        "12": { "inputs": { "images": ["28", 0], "filename_prefix": "WAI_Illustrious_2K" }, "class_type": "SaveImage" }
    },
