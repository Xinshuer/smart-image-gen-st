...
export const workflowTemplates = {
    ...
    // v0.11.12 接入 DMD2 4step LoRA: 8 steps + lcm sampler + simple scheduler + CFG 1.6 + 0.7 strength
    // 速度: 30 步 → 8 步，约 4× 提速。质量: 接近原版，PONY-XL 系列 DMD2 兼容性最好。
    wai_anihentai: {
        "1": { "inputs": { "ckpt_name": "waiANIHENTAIPONYXL_v60.safetensors", "clip_skip": "2" }, "class_type": "CheckpointLoaderSimple" },
        "5": { "inputs": { "model": ["1", 0], "clip": ["1", 1], "lora_name": "dmd2_sdxl_4step_lora_fp16.safetensors", "strength_model": 0.7, "strength_clip": 0.7 }, "class_type": "LoraLoader" },
        "7": { "inputs": { "clip": ["5", 1], "text": "%prompt%" }, "class_type": "CLIPTextEncode" },
        "8": { "inputs": { "clip": ["5", 1], "text": "%negative_prompt%" }, "class_type": "CLIPTextEncode" },
        "9": { "inputs": { "width": 832, "height": 1216, "batch_size": 1 }, "class_type": "EmptyLatentImage" },
        "10": { "inputs": { "model": ["5", 0], "positive": ["7", 0], "negative": ["8", 0], "latent_image": ["9", 0], "seed": 0, "steps": 8, "cfg": 1.6, "sampler_name": "lcm", "scheduler": "simple", "denoise": 1.0 }, "class_type": "KSampler" },
    ...
    // v0.11.12 unholy_desire 已经是自蒸馏 16 步配置，DMD2 叠加可能过度蒸馏。
    // **保持不接 DMD2**，维持原作者推荐 (dpmpp_2m karras CFG 2.5 16 步)。仍然快。
    unholy_desire: {
    ...
    // Nova Orange XL Rex v1.0 (novaOrangeXL_rexV10) — Illustrious 派生 anime 模型 (v0.11.20)。
    // 作者推荐: euler_a + 20-30 steps + CFG 3-5；PREFIX 含 newest/very aesthetic/ultra-detailed；
    //   NEGATIVE 含 anti-old/cartoon/text/anti-conjoined + (worst quality, bad quality:1.2)。
    // v0.11.20 接入 DMD2 4step LoRA（与其他 5 个 IL/Pony 工作流一致）：
    //   8 steps + lcm + simple + CFG 1.6 + DMD2 weight 0.7。
    //   作者满血路径 (20-30 步 CFG 3-5) 若需要可手动切到 unholy_desire (16 步 CFG 2.5 无 DMD2)。
    // clip_skip="2"（IL 标准；不复用 nova_asian_il 的 clip_skip="1"，那个是为压制色偏的特殊配置）。
    nova_orange_xl: {
        "1": { "inputs": { "ckpt_name": "novaOrangeXL_rexV10.safetensors", "clip_skip": "2" }, "class_type": "CheckpointLoaderSimple" },
        "5": { "inputs": { "model": ["1", 0], "clip": ["1", 1], "lora_name": "dmd2_sdxl_4step_lora_fp16.safetensors", "strength_model": 0.7, "strength_clip": 0.7 }, "class_type": "LoraLoader" },
        "7": { "inputs": { "clip": ["5", 1], "text": "%prompt%" }, "class_type": "CLIPTextEncode" },
        "8": { "inputs": { "clip": ["5", 1], "text": "%negative_prompt%" }, "class_type": "CLIPTextEncode" },
        "9": { "inputs": { "width": 832, "height": 1216, "batch_size": 1 }, "class_type": "EmptyLatentImage" },
        ...
        "27": { "inputs": { "model_name": "RealESRGAN_x2plus.pth" }, "class_type": "UpscaleModelLoader" },
        "28": { "inputs": { "upscale_model": ["27", 0], "image": ["31", 0] }, "class_type": "ImageUpscaleWithModel" },
        "12": { "inputs": { "images": ["28", 0], "filename_prefix": "NovaOrange_XL_2K" }, "class_type": "SaveImage" }
    },
