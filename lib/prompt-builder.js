...
// 会产生强 ecchi prior：把"心形/星形乳贴"塞进画面作"模糊化覆盖"。ANATOMY_QUALITY_NEG_ANIME
// 里有 (pasties:1.5) 总词但 heart/star 变体无加权 → 漏过此 prior。
// 这个常量给 heart/star/decorative 变体单独高加权，仅在 buildReferencePromptFull 注入
// （参考图必走 SFW，伪影集中处；消息内图 SFW 极少撞这组合，不需要叠 boost）。
const ANTI_PASTIES_BOOST = '(heart pasties:1.8), (star pasties:1.8), (heart-shaped nipple cover:1.7), (decorative nipple cover:1.6), (pasty:1.6), (nipple pasties:1.6), (chest sticker:1.5), heart shape on breast, heart shape on chest, star shape on breast, decorative chest, decorative breasts, fancy nipple cover, ornate nipple cover, ecchi pasties, x-shaped pasties, cross-shaped pasties, sticker on breast, sticker on chest';
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (isAnimeModel(m)) {
        ...
        // v0.11.24 加 ANTI_PASTIES_BOOST 强化 anime 模型心形/星形乳贴压制
