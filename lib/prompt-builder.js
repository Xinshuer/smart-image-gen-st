...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (isAnimeModel(m)) {
        ...
        // v0.11.26 ⭐ 加 FORCE_FULL_COVERAGE 物理消除 cleavage 暴露区，装饰物无落脚点
        positive = `${prefix}, ${ratingTag}${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_ANIME}, ${SFW_POSITIVE_BONUS}, ${PLAIN_SKIN_POSITIVE}, ${FORCE_FULL_COVERAGE_POSITIVE}`;
        // v0.11.21 加 ANTI_PATTERN_BLEED 防旗袍花纹印到皮肤
        // v0.11.24/v0.11.25 加 ANTI_PASTIES_BOOST 扩展版（含中国结/盘扣/medallion 等所有装饰物变体）
        // v0.11.26 加 FORCE_FULL_COVERAGE_NEGATIVE 反所有露胸/cleavage 表现
        negative = `${FORCE_FULL_COVERAGE_NEGATIVE}, ${ANTI_PASTIES_BOOST}, ${NEGATIVE_EMBEDDINGS_SDXL}, ${ANATOMY_QUALITY_NEG_ANIME}, ${CLOTHES_BODY_NEG}, ${ANTI_MULTI_CHAR_NEG}, ${ANTI_PATTERN_BLEED_NEG}, ${STRONG_SFW_NEGATIVE}, ${NEGATIVE[m]}`;
    } else {
