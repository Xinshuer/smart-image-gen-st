...
const PREFIX = {
    // v0.11.1 删除人种偏置 'asian' tag —— 用户决定不再指定人种，让模型 prior + 角色锚自决定。
    asian_realism: 'score_9, score_8_up, score_7_up, masterpiece, best quality, ultra detailed, photorealistic, raw photo, sharp focus, detailed skin, daylight, natural lighting, soft lighting, white background, indoor lighting, even lighting',
    ...
    // Nova Asian XL Illustrious v5.0 — v0.11.5 重新定位为写实模型（clip_skip=1 + hires fix 能出写实）。
    // 严格按作者 Illustrious 模板（含 BREAK 关键字分段）：
    //   "masterpiece, ..., {Prompt}, BREAK, depth of field, photorealistic details"
    // BREAK 在 prompt-builder buildPrompt 末端通过特殊标记 __NOVA_BREAK__ 处理（拼接后替成 BREAK）
    nova_asian_il: 'masterpiece, best quality, amazing quality, very aesthetic, high resolution, ultra-detailed, absurdres, scenery, photorealistic',
};
...
const TECH = {
    ...
    wai_anihentai: { cfg: 7.0, sampler: 'euler_ancestral', scheduler: 'karras', steps: 30 },
    // unholyDesireMixSinister v80 作者推荐: 16+ steps + CFG 2.5+ + DPM++ 2M Karras
    unholy_desire: { cfg: 2.5, sampler: 'dpmpp_2m', scheduler: 'karras', steps: 16 },
    // divingIllustriousFlat v70VAE 作者推荐: 25 steps + CFG 4-7 + Euler a + Karras + ADetailer
    diving_illustrious: { cfg: 5.0, sampler: 'euler_ancestral', scheduler: 'karras', steps: 25 },
    // LUSTIFY v8 Apex 作者推荐: DPM++ 2M SDE + Karras/Exponential + 30 steps + CFG 2.5-4.5
    lustify_v8: { cfg: 3.5, sampler: 'dpmpp_2m_sde', scheduler: 'karras', steps: 30 },
    // Nova Asian XL Illustrious 作者推荐: Euler a + 20-40 steps + CFG 3-5
    nova_asian_il: { cfg: 4.0, sampler: 'euler_ancestral', scheduler: 'karras', steps: 30 },
};

// v0.11.5 isAnimeModel — nova_asian_il 移回写实组（实测 clip_skip=1 + hires fix 后能出写实）
// anime 3 个：wai_anihentai / unholy_desire / diving_illustrious
// 写实 3 个：asian_realism / lustify_v8 / nova_asian_il
function isAnimeModel(m) {
    return m === 'wai_anihentai' || m === 'unholy_desire' || m === 'diving_illustrious';
}
...
// ────────────────────────────────────────────────────────────────────────
// v0.11.6 LUSTIFY 专用"斯拉夫少女 / 东欧 selfie"风格美貌增强
// 用户反馈：v0.11.6a 用 "high cheekbones / runway model" 等 Kardashian 重妆派，"更丑了"。
// 用户实际想要：东欧/斯拉夫 doll-like 清纯派，参考 Pinterest/TikTok girl 美学。
// 关键反差：
//   - 不要：cheekbones / jawline / contoured / supermodel / runway （硬朗轮廓）
//   - 要：delicate / soft / fresh-faced / youthful / dewy / natural（柔嫩水润）
// 控制加权数 ≤3 防 CLIP 过载（v0.11.6a 用 4 个加权，可能也是失败因素之一）。
const LUSTIFY_BEAUTY_BOOST = '(beautiful face:1.3), (delicate features:1.2), (slavic beauty:1.2), eastern european, fresh-faced, natural beauty, youthful face, soft features, doll-like face, pretty face, gentle expression, dewy skin, glowing skin, smooth skin, natural makeup, no heavy makeup, pinterest girl, candid selfie';
...
const ASIAN_TO_WESTERN_MAP = [
    ...
    // 皮肤 — porcelain 是亚洲审美词
    [/\bporcelain skin\b/g, 'fair skin, freckles'],
    [/\bporcelain\b/g, 'freckles'],
    // 五官 — delicate nose 偏东亚
    [/\bdelicate nose\b/g, 'high nose bridge'],
    // 民族服装 — 中式日式传统服转西方等价
...
export function buildPrompt({ aiPrompt = '', characterAnchor = '', characterFullPrompt = '', intent = { level: 'sfw', tags: [] }, model = 'wai_anihentai', styleHint = 'auto' }) {
    ...
    const VALID_MODELS = ['asian_realism', 'wai_anihentai', 'unholy_desire', 'diving_illustrious', 'lustify_v8', 'nova_asian_il'];
    ...
    // v0.11.5 Nova Asian XL：尾部加 BREAK + photorealistic details（作者 Illustrious 模板要求）
    // BREAK 关键字让 ComfyUI CLIP 把 prompt 分两段编码，第二段专注 photo detail。
    if (m === 'nova_asian_il') {
        positive = `${positive}, BREAK, depth of field, photorealistic details`;
    }
...
export function buildGroupPrompt({
    ...
    // v0.11.5 Nova Asian 群聊也加 BREAK + photo tail
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (isAnimeModel(m)) {
        ...
        positive = `${PREFIX[m]}, ${ratingTag}${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_ANIME}`;
        negative = `${NEGATIVE_EMBEDDINGS_SDXL}, ${ANATOMY_QUALITY_NEG_ANIME}, ${CLOTHES_BODY_NEG}, ${ANTI_MULTI_CHAR_NEG}, ${STRONG_SFW_NEGATIVE}, ${NEGATIVE[m]}`;
    } else {
        // 写实分支 — asian_realism / lustify_v8 共用 realistic 路径（nova_asian_il 已移到 anime）
        positive = `${PREFIX[m]}, ${ratingTag}${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_REALISTIC}`;
        negative = `${NEGATIVE_EMBEDDINGS_SDXL}, ${ANATOMY_QUALITY_NEG_REALISTIC}, ${CLOTHES_BODY_NEG}, ${ANTI_MULTI_CHAR_NEG}, ${STRONG_SFW_NEGATIVE}, ${NEGATIVE[m]}`;
    }
    // v0.11.3 LUSTIFY 参考图也 westernize
    if (m === 'lustify_v8') {
        positive = westernizePrompt(positive);
        positive = `${positive}, ${LUSTIFY_BEAUTY_BOOST}`;
    }
