...
const NEGATIVE = {
    ...
    // Nova Asian XL Illustrious v5.0 — v0.11.2 改走 anime 路径（实测是动漫模型）。
    // negative 不再反 anime/cartoon/painting（那些是模型本身风格），只反基础 deformity。
...
const PREFIX = {
    ...
    // Nova Asian XL Illustrious v5.0 — 实测是动漫模型（不是 photo），作者宣传误导。
    // v0.11.2 移到 anime 组：去掉 photo PREFIX，用 anime 标准词。
...
// v0.11.2 isAnimeModel — 6 个模型中 4 个 anime 走同一 anatomy 路径（防 balloon 胸/校服胸贴等）
// 写实 2 个：asian_realism / lustify_v8 (NOT Pony/Illustrious 派生)
// v0.11.2 nova_asian_il 实测纯动漫输出（Illustrious 基础推不动到 photo），
//         移到 anime 组——作者宣传 photo-realistic 是误导，骨子里是 Illustrious 动漫。
function isAnimeModel(m) {
    return m === 'wai_anihentai' || m === 'unholy_desire' || m === 'diving_illustrious' || m === 'nova_asian_il';
}
...
export function buildPrompt({ aiPrompt = '', characterAnchor = '', characterFullPrompt = '', intent = { level: 'sfw', tags: [] }, model = 'wai_anihentai', styleHint = 'auto' }) {
    ...
    if (m === 'lustify_v8') {
        ...
        // Also westernize negative — 防 anti-asian tag 漏写
        // 但 negative 里大多是 quality 词，影响不大。这里只做正面。
    }
