...
const PREFIX = {
    ...
    // Nova Asian XL Illustrious v5.0 — 作者推荐 Illustrious PREFIX 完整版。
    // v0.11.1 删除 'asian' 人种偏置 tag —— 模型本身亚洲人脸训练已自带 prior，无需额外 bias。
    nova_asian_il: 'masterpiece, best quality, amazing quality, very aesthetic, high resolution, ultra-detailed, absurdres, scenery, photorealistic, depth of field, photorealistic details, raw photo, sharp focus, detailed skin',
};
...
// v0.11.0 isAnimeModel — 6 个模型中 3 个 anime 走同一 anatomy 路径（防 balloon 胸/校服胸贴等）
// 写实 3 个：asian_realism / lustify_v8 (NOT Pony/Illustrious 派生) / nova_asian_il (Illustrious 派生)
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (isAnimeModel(m)) {
        ...
        // 写实分支 — asian_realism / lustify_v8 / nova_asian_il 共用 realistic 路径
