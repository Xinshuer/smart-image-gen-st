...
export function buildPrompt({ aiPrompt = '', characterAnchor = '', characterFullPrompt = '', intent = { level: 'sfw', tags: [] }, model = 'wai_anihentai', styleHint = 'auto', userText = '', loraTriggerText = '' }) {
    ...
    // v0.11.28 偷拍检测 — 用户明确说偷拍/偷看/窥视/voyeur 时，AI 经常仍写 selfie/looking at
    // viewer/holding phone，把场景渲染成自拍。检测到 → 从 AI prompt strip selfie 类 tag +
    // 注入 voyeur 正面 + anti-selfie 负面。
    const VOYEUR_RE = /偷拍|偷看|窥视|窥探|偷窥|偷.{0,3}照|voyeur|peeking|peeping|hidden camera/i;
    const triggerText = (loraTriggerText || userText || '') + ' ' + (aiPrompt || '');
    const isVoyeur = VOYEUR_RE.test(triggerText);
    if (isVoyeur) {
        // Strip selfie/looking-at-viewer 类 tag — 偷拍场景人物应该不看镜头
        const ANTI_SELFIE_TOKENS = ['selfie', 'mirror selfie', 'looking at viewer', 'eye contact', 'looking at camera', 'holding phone', 'phone in hand', 'phone selfie', 'posed photo', 'posing for camera', 'smiling at viewer'];
        const lowerTokens = ANTI_SELFIE_TOKENS.map(t => t.toLowerCase());
        cleanedAiPrompt = cleanedAiPrompt.split(',').map(t => t.trim()).filter(Boolean).filter(tag => {
    ...
    // v0.11.28 偷拍正面 tag — 注入 voyeur 视角描述（替代 selfie 视角）
    if (isVoyeur) {
        parts.push('(voyeur:1.3), (hidden camera shot:1.3), (candid photo:1.2), peeping shot, from hidden angle, unaware subject, not looking at camera, looking away, caught off guard, secret photo');
        // 负面强力反 selfie/posing
        negative = `(selfie:1.5), (looking at viewer:1.5), (eye contact:1.5), (holding phone:1.5), (mirror selfie:1.5), (posed photo:1.4), posing for camera, smiling at viewer, ${negative}`;
    }
...
// v0.11.27 reduce weights 1.3 → 1.1 — force-stacking high weights 是装饰物伪影的元凶之一
// 模仿 AI 在 chat 路径写 prompt 的策略：温和而非高压
const SFW_POSITIVE_BONUS = '(intact clothing:1.1), (covered breasts:1.1), (clothed:1.1), fully clothed, no exposed breast, modest attire';
...
// v0.11.26 ⭐⭐ Force Full Coverage — 终极武器：消除 cleavage 暴露区
// 用户反复实测 anti-decoration / strip-pattern / plain-skin 都压不住 ecchi prior。
// 改策略：物理消除"乳头/cleavage 暴露区" → 模型没有装饰目标皮肤 → 装饰物 prior 失去落脚点。
// 强制旗袍渲染为传统全包高领封闭风格（非现代低领露胸 style）。
const FORCE_FULL_COVERAGE_POSITIVE = '(closed neckline:1.5), (high neckline:1.5), (covered cleavage:1.5), (buttoned up:1.4), (conservative neckline:1.4), (high collar:1.4), (fully clothed chest:1.4), no chest exposed, modest coverage, traditional collar, fabric covers chest';
const FORCE_FULL_COVERAGE_NEGATIVE = '(cleavage:1.5), (visible cleavage:1.5), (deep cleavage:1.5), (low cut:1.5), (plunging neckline:1.5), (open neckline:1.4), (boob window:1.5), (chest cutout:1.5), (breast cutout:1.5), exposed chest, exposed cleavage, chest visible, breast visible, sideboob, underboob';

// v0.11.26 anchor 路径要 strip 的 cleavage 相关 tag — 跟 DECORATIVE_TOKENS_TO_STRIP 共用 stripper 走同一遍
const CLEAVAGE_TOKENS_TO_STRIP = [
    'cleavage', 'large cleavage', 'deep cleavage', 'huge cleavage', 'massive cleavage',
    'boob window', 'underboob', 'sideboob', 'shoulder boob',
    'chest cutout', 'breast cutout', 'cutout',
    'plunging neckline', 'low neckline', 'open neckline', 'low cut',
    'cleavage cutout', 'exposed chest', 'exposed cleavage',
];

function stripCleavageTokens(prompt) {
    if (!prompt) return '';
    const lowerTokens = CLEAVAGE_TOKENS_TO_STRIP.map((t) => t.toLowerCase());
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    // v0.11.26 anime 路径加 strip cleavage tag — 物理消除 cleavage 暴露区，装饰物无处可贴
    let cleaned = processPromptForModel(stripNsfwTokens(sdPrompt), m);
    if (isAnimeModel(m)) {
        cleaned = stripDecorativePatterns(cleaned);
        cleaned = stripCleavageTokens(cleaned);
    }
    // v0.11.27 ⭐ 学 AI 在 chat 路径的 prompt 策略 — 改 upper_body → cowboy shot + 场景上下文
    // 原因：upper_body 强制把胸部推到画面 60%+，模型注意力全集中到 cleavage 区 → ecchi 装饰物
    // prior 强触发。cowboy shot (waist up 但站立) + 场景上下文（outdoor / natural environment）
    // 让注意力分散到全身 + 背景 → 模型不会在乳头位置放东西。
    // 对比 AI 自己的 chat prompt 总是带"outdoor, mountain, cloudy sky" 这类 scene tag，效果干净。
    const PORTRAIT_FRAMING = '(looking at viewer:1.2), cowboy shot, standing pose, outdoor scenic background, natural environment';
    ...
    if (isAnimeModel(m)) {
        ...
        // v0.11.27 ⭐ 大幅 simplify — 删 v0.11.25 PLAIN_SKIN_POSITIVE / v0.11.26 FORCE_FULL_COVERAGE_*
        // 原因：force-stacking 反而触发 ecchi prior，AI 的 chat prompt 干净是因为 NOT force-stack。
        // 学 AI 策略：温和 SFW 压力（weights 1.1）+ cowboy shot 散开注意力 + 场景上下文。
        // 保留 ANTI_PASTIES_BOOST + ANTI_PATTERN_BLEED 兜底（万一漏过）。
        positive = `${prefix}, ${ratingTag}${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_ANIME}, ${SFW_POSITIVE_BONUS}`;
