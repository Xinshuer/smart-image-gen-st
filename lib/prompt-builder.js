...
// v0.10.3 加 (intact nipples:1.3) 主动反 Illustrious 模型 ecchi prior 触发的 heart/star pasties
//         - intact nipples 仅反 nipple cover/pasties，不阻止 NSFW 裸露
//         - 配 plain breast skin / no chest decoration 无加权暗示
const ANATOMY_QUALITY_POS_ANIME = '(solo:1.4), (1girl:1.4), (perfect anatomy:1.2), detailed body, pointed chin, beautiful face, slender curves, (natural breast shape:1.4), (intact nipples:1.3), plain breast skin, no chest decoration, clean breasts, balanced proportions, proportionate breasts, aesthetic body, body harmony, breasts smaller than torso';
...
function processPromptAnime(p) {
    return stripHeartTokens(dampenImplicitBreastBoost(capBreastWeight(p, 1.0), 0.7, IMPLICIT_BOOST_TOKENS_ANIME));
}
function processPromptRealistic(p) {
    return stripHeartTokens(dampenImplicitBreastBoost(capBreastWeight(p, 1.2), 0.85, IMPLICIT_BOOST_TOKENS_REALISTIC));
}
function processPromptForModel(p, model) {
    return isAnimeModel(model) ? processPromptAnime(p) : processPromptRealistic(p);
}

// v0.10.2 stripHeartTokens — 修 Illustrious 模型 "heart" 关键词扩散 bug
// 卡片视觉档案常用 booru 标准 tag `heart-shaped ass` (沙漏臀)，但 SDXL Illustrious
// 见到 heart 关键词会把心形概念扩散到全身 → 胸部出现 heart 图案 / heart pasties。
// 不动卡片内容（保留意图），在 prompt 进 SDXL 前自动转成无 heart 的同义词。
function stripHeartTokens(prompt) {
    if (!prompt || typeof prompt !== 'string') return prompt;
    return prompt
        // heart-shaped ass → 等效曲线 tag
        .replace(/heart[-\s]shaped\s+ass/gi, 'bubble butt')
        // 通用 heart-shaped → rounded
        .replace(/heart[-\s]shaped/gi, 'rounded')
        // (heart pasties:1.5) 等加权 tag 整个删
        .replace(/\([^()]*\bheart\b[^()]*:[\d.]+\)/gi, '')
        // 任何含 heart 的 plain tag (逗号分隔) 整个删
        .replace(/(?:^|,)[^,]*\bheart\b[^,]*/gi, '')
        // 清理多余逗号空格
        .replace(/,\s*,/g, ',')
        .replace(/^\s*,\s*/, '')
...
const PREFIX = {
    ...
    diving_illustrious: '(anime coloring:1.4), (anime screencap:1.4), masterpiece, best quality, amazing quality, very aesthetic, absurdres, ultra detailed face',
};
...
// v0.10.1 isAnimeModel — 4 个模型中 3 个 anime 走同一 anatomy 路径（防 balloon 胸/校服胸贴等）
...
export function buildPrompt({ aiPrompt = '', characterAnchor = '', characterFullPrompt = '', intent = { level: 'sfw', tags: [] }, model = 'wai_anihentai', styleHint = 'auto' }) {
    ...
    const VALID_MODELS = ['asian_realism', 'wai_anihentai', 'unholy_desire', 'diving_illustrious'];
    ...
    // v0.10.2 也对 AI 写的 pic prompt 做 heart 剥离（AI 可能复用了 heart-shaped 等 tag）
    cleanedAiPrompt = stripHeartTokens(cleanedAiPrompt);
...
export function buildGroupPrompt({
    ...
    const styleSteer = m === 'wai_anihentai'
        ? 'anime style, illustration, group photo, looking at viewer'
        : 'photorealistic, group photo, asian, looking at viewer, natural lighting';

    const cleanedAiPrompt = stripHeartTokens(nsfw ? aiPrompt : stripNsfwTokens(aiPrompt));
    const positiveParts = [
        prefix,
        nsfw ? 'rating_explicit' : 'rating_safe',
        multiTag,
        styleSteer,
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (m === 'asian_realism') {
        ...
        // 各模型 PREFIX/NEGATIVE 不同，按 m 索引
        positive = `${PREFIX[m]}, rating_safe, ${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_ANIME}`;
