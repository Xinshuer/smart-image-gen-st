...
// v0.8.0 大瘦身：删 6 个旧模型 NEGATIVE。只保留 asian_realism + wai_anihentai。
const NEGATIVE = {
    // Asian Realism by Stable (PONY-based) — 前置 Stable_Yogis_PDXL_Negatives-neg trigger 词激活 negative LoRA。
...
// v0.8.0 大瘦身：删 6 个旧模型 PREFIX/SIZE/TECH。只保留 asian_realism + wai_anihentai。
const PREFIX = {
...
export function buildReferencePromptFull({ sdPrompt = '', model = 'wai_anihentai' }) {
    ...
    if (m === 'asian_realism') {
        // 必须前置正面 trigger 词激活 LoRA + rating_safe
        positive = `Stable_Yogis_PDXL_Positives, rating_safe, ${cleaned}, ${PORTRAIT_FRAMING}, ${ANATOMY_QUALITY_POS_REALISTIC}`;
